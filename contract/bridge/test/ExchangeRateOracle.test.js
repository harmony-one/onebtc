...
contract("ExchangeRateOracle unit test", (accounts) => {
  ...
  it ("add authorized oracle", async function () {
    await this.ExchangeRateOracleWrapper.addAuthorizedOracle(this.authorizedOracle);

    let isAuthorizedOracle = await this.ExchangeRateOracleWrapper.authorizedOracles(this.authorizedOracle);
    assert.equal(isAuthorizedOracle, true);
    isAuthorizedOracle = await this.ExchangeRateOracleWrapper.authorizedOracles(this.unauthorizedOracle);
  ...
  it("setExchangeRate, getExchangeRate", async function () {
    ...
    const req = await this.ExchangeRateOracleWrapper.setExchangeRate(this.authorizedOracle, 100);
  ...
  it("Error on setExchange with unauthorizedOracle", async function () {
    await expectRevert(this.ExchangeRateOracleWrapper.setExchangeRate(this.unauthorizedOracle, 200), 'ERR_INVALID_ORACLE_SOURCE');
  });

  it("setSatoshiPerBytes", async function () {
    const req = await this.ExchangeRateOracleWrapper.setSatoshiPerBytes(5, 10, { from: this.authorizedOracle });
  ...
  it("Error on setSatoshiPerBytes with unauthorizedOracle", async function () {
    await expectRevert(this.ExchangeRateOracleWrapper.setSatoshiPerBytes(5, 10, { from: this.unauthorizedOracle }), 'ERR_INVALID_ORACLE_SOURCE');
  });
