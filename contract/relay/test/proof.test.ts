...
describe("Proofs", () => {
  let relay: Relay;
  let genesisHeader = "0x0000c020c238b601308b7297346ab2ed59942d7d7ecea8d23a1001000000000000000000b61ac92842abc82aa93644b190fc18ad46c6738337e78bc0c69ab21c5d5ee2ddd6376d5d3e211a17d8706a84"; // 592920
  let tx = {
    version: "0x01000000",
  ...
  it("should fail with insufficient confirmations", async () => {
    relay = await deploy(genesisHeader, 5);
  ...
  it("should fail with block not found", async () => {
    relay = await deploy(genesisHeader, 100);
  ...
  it("should validate inclusion", async () => {
    relay = await deploy(genesisHeader, 1);
  ...
  it("should validate empty block coinbase", async () => {
    ...
    relay = await deploy(header, height);
  ...
  for (let i = 0; i < networks.length; i++) {
    ...
    it("should validate inclusion on " + network.type, async () => {
      relay = await deploy(network.header, network.height);
      expect(await getBestBlockHeight(relay)).to.eq(network.height);

      let proof = network.intermediateNodes.map((value) => Buffer.from(value, 'hex').reverse().toString('hex')).join("");
      let txid = Buffer.from(network.txId, 'hex').reverse().toString('hex');

      await relay.verifyTx(network.height, network.index, "0x" + txid, network.header, "0x" + proof, 0, true);
    })
  }
});