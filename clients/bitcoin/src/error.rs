...
impl Error {
    pub fn is_connection_refused(&self) -> bool {
        matches!(self,
            Self::BitcoinError(BitcoinError::JsonRpc(JsonRpcError::Hyper(HyperError::Io(err))))
    ...
    pub fn is_json_decode_error(&self) -> bool {
        matches!(
            self,
            Self::BitcoinError(BitcoinError::JsonRpc(JsonRpcError::Json(_)))
        )
    }

    pub fn is_wallet_error(&self) -> bool {
        matches!(self,
            Self::BitcoinError(BitcoinError::JsonRpc(JsonRpcError::Rpc(err)))
